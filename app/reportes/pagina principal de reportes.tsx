...
// Definición de tipos antes del componente

type ReporteCliente = { name: string; cantidad: number };

type ReporteData = ReporteFinanciero | ReporteCliente;
...
export default function ReportesPage() {
  ...
  reporteClientes: ReporteClientes;
    ...
    reporteFinanciero: { ingresos: 0, gastos: 0, utilidad: 0, crecimiento: 0 },
    reporteClientes: { activos: 0, nuevos: 0, bajas: 0, total: 0 },
  })
  ...
  try {
    ...
    // Aquí asignamos las propiedades correspondientes de los datos recibidos
    if (data.reporteFinanciero && data.reporteClientes) {
      setReporteData({
        reporteFinanciero: {
          ingresos: data.reporteFinanciero.ingresos,
          gastos: data.reporteFinanciero.gastos,
          utilidad: data.reporteFinanciero.utilidad,
          crecimiento: data.reporteFinanciero.crecimiento,
        },
        reporteClientes: {
          activos: data.reporteClientes.activos,
          nuevos: data.reporteClientes.nuevos,
          bajas: data.reporteClientes.bajas,
          total: data.reporteClientes.total,
        },
      })
    }
  } catch (err) {
...
const exportarReporte = () => {
  ...
  const reportData = reportType === "general" ? datosFinancieros : datosClientes;

  const csvRows = [];
  // Encabezados
  csvRows.push(["Categoria", "Monto/Cantidad"]);
  
  // Filas de datos, discriminar si es monto o cantidad
  reportData.forEach((row) => {
    if ('monto' in row) {
      csvRows.push([row.name, row.monto]);
    } else if ('cantidad' in row) {
      csvRows.push([row.name, row.cantidad]);
    }
  });
  ...
  // Crear un archivo Blob y trigger para descargar
  const blob = new Blob([csvString], { type: "text/csv" });
  const url = window.URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `reporte_${selectedMonth}_${selectedYear}_${reportType}.csv`;
  a.click();
